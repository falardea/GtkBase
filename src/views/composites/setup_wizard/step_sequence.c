...
struct _StepSequence
{
   ...
   void (*on_sequence_start)(SequenceRunner *parent_sequence, gpointer user_data);
   gpointer callback_user_data;

   SequenceRunner *parent_sequence;
};

static void step_sequence_execute(StepExecutable *self)
{
   ...
   gtk_widget_set_sensitive(GTK_WIDGET(sa->btn_start_sequence), TRUE);
}
...
void on_btn_start_sequence_clicked(__attribute__((unused)) GtkButton *button, gpointer user_data)
{
   logging_llprintf(LOGLEVEL_DEBUG, "%s", __func__);
...
static void step_sequence_class_init(StepSequenceClass *klass)
{
   ...
   gtk_widget_class_bind_template_child(widget_class, StepSequence, btn_start_sequence);

   gtk_widget_class_bind_template_callback_full(widget_class, "on_btn_start_sequence_clicked", (GCallback)on_btn_start_sequence_clicked);
}
...
StepSequence *step_sequence_new(const gchar *step_description,
                                      ...
                                      SequenceRunner *parent_sequence,
                                      SequenceCallback_T on_sequence,
                                      gpointer callback_user_data)
{
   logging_llprintf(LOGLEVEL_DEBUG, "%s", __func__);
   StepSequence *sa;
   ...
   gtk_button_set_label(sa->btn_start_sequence, btn_label_str);
