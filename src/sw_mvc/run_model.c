...
struct _RunModel
{
   ...
   RUN_SETUP_PHASES  last_completed_phase;
...
static guint      run_model_mode_change_sig[RUN_MODEL_N_SIGNALS] = {0, };
...
static void run_model_class_init( RunModelClass *klass )
{
   ...
   model_properties[RUN_MODEL_PROP_RUN_MODE] = g_param_spec_uint("run-mode",
                                                          "Run Run Mode",
                                                          "The run run mode of the model",
                                                          RUN_MODE_NOT_SET, N_RUM_MODEL_MODES-1, RUN_MODE_NOT_SET,
                                                          G_PARAM_READWRITE );
   g_object_class_install_properties( gobject_class, RUN_MODEL_N_PROPERTIES, model_properties);

   run_model_mode_change_sig[RUN_MODEL_SIGNAL_MODE_CHANGE] = g_signal_new_class_handler(RUN_MODEL_MODE_CHANGE_SIGNAL_STR,
...
RUN_SETUP_PHASES run_model_get_last_completed_phase(RunModel *self )
{
   ...
   g_return_val_if_fail(self->last_completed_phase < RUN_SETUP_N_STEPS, RUN_SETUP_FAILED);
   logging_llprintf(LOGLEVEL_DEBUG, "%s", __func__);
